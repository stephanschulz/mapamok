...
int findNearestVertex(const vector<glm::vec3>& vertices, const ofVec3f& base) {
    int nearestIndex = 0;
...
ofMesh mergeNearbyVertices(const ofMesh& mesh, float tolerance = 0) {
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(mergedMesh.getNumVertices() > 0) {
            ...
            const glm::vec3& nearestVertex = mergedMesh.getVertices()[nearestIndex];
...
void project(ofMesh& mesh, const ofCamera& camera, ofRectangle viewport) {
    glm::mat4 modelViewProjectionMatrix = camera.getModelViewProjectionMatrix(viewport);
        
        viewport.width /= 2;
        viewport.height /= 2;
        for (int i = 0; i < mesh.getNumVertices(); i++) {
            glm::vec3& cur = mesh.getVerticesPointer()[i];
            glm::vec4 CameraXYZ = modelViewProjectionMatrix * glm::vec4(cur, 1.0f);
            cur.x = (CameraXYZ.x / CameraXYZ.w + 1.0f) * viewport.width + viewport.x;
            cur.y = (1.0f - CameraXYZ.y / CameraXYZ.w) * viewport.height + viewport.y;
            cur.z = CameraXYZ.z / CameraXYZ.w / 2;
        }
}
...
void buildNormalsAverage(ofMesh& mesh) {
    auto getNormal = [](const glm::vec3& p0, const glm::vec3& p1, const glm::vec3& p2) {
            glm::vec3 u = p1 - p0;
            glm::vec3 v = p2 - p0;
        ...
        vector<ofIndexType>& indices = mesh.getIndices();
        vector<glm::vec3> normals(mesh.getNumVertices(), glm::vec3(0.0f, 0.0f, 0.0f));
